...
exports.format = (parsedQuery) => {
  ...
  if (parsedQuery.header) {
    addLine(parsedQuery.header);
  }
  forPrologue(parsedQuery.prologue);
  parsedQuery.functions.forEach(addFunction);
...
forGraphPattern = (pattern, indent) => {
  if (pattern.token === 'basicgraphpattern') {
    ...
    addLine('OPTIONAL {');
    addGroupGraphPattern(pattern.value, indent + indentUnit);
    addLine('}');
...
addFunction = (func) => {
  ...
  addLine(`${name}(${args}) {`);
  addGroupGraphPattern(func.body, currentIndent + indentUnit);
  addLine('}');
  addLine('');
};
