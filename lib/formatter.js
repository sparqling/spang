...
const addGroupGraphPattern = (pattern) => {
  increaseIndent();
  pattern.patterns.forEach((p) => {
    switch (p.token) {
      case 'graphunionpattern':
        addLine('{');
        addGroupGraphPattern(p.value[0]);
        addLine('}');
        for (let i = 1; i < p.value.length; i++) {
          addLine('UNION');
          addLine('{');
          addGroupGraphPattern(p.value[i]);
          addLine('}');
        }
        break;
      case 'optionalgraphpattern':
        addLine('OPTIONAL {');
        addGroupGraphPattern(p.value);
        addLine('}');
        break;
      ...
      case 'expression':
        if (p.expressionType === 'functioncall') {
          var name = getUri(p.iriref);
          var args = p.args.map(getExpression).join(', ');
          addLine(`${name}(${args})`);
        } else {
          debugPrint(p);
        }
