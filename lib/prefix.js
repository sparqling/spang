...
exports.expandPrefixedUri = (arg) => {
  let match = /^(\S+?)@github:([^\/]+)\/([^\/]+)\/(.+)/.exec(arg);
  if (match) {
    return `https://raw.githubusercontent.com/${match[2]}/${match[3]}/${match[1]}/${match[4]}`;
  }
  match = /^https:\/\/github.com\/([^\/]+)\/([^\/]+)\/blob\/(.+)/.exec(arg);
  if (match) {
    return `https://raw.githubusercontent.com/${match[1]}/${match[2]}/${match[3]}`;
  }
  else if (/^(http|https):\/\//.test(arg)) {
    return arg;
  } else if (/^\w+:/.test(arg)) {
    const matched = arg.match(/^(\w+):(.*)$/);
    return expandPrefix(matched[1]) + matched[2];
  } else {
