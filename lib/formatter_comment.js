...
addToken = (text, origLine, forceLineBreak = false, addSpace = true) => {
  ...
  while(Comments.length > 0 &&
    ...
    else if(origLine > prevOrigLine + 1 && prevOrigLine != Comments[0].line) lines.push(Comments[0].text);
...
forExpression = (expression) => {
  ...
  case 'builtincall':
    ...
    expression.args.forEach((arg) => {
      if(!first) addToken(',', -1, false, false);
      forExpression(arg);
      first = false;
    });
    decreaseIndent();
  ...
  case 'irireforfunction':
    ...
    break;
  default:
    addToken('expressionType: ' + expression.expressionType); // for debug
  }
};
